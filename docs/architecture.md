...
Runtime (@ongkirhub/runtime)
  -> provider registry resolution
  -> quote orchestration across providers
  -> request validation before provider dispatch
  -> health reporting
...
- `apps/api` -> `core`, `runtime`, `provider-*`
- `runtime` -> `core`
- `provider-*` -> `core`
...
**Current (v0.1):** provider orchestration lives in `@ongkirhub/runtime`. The HTTP adapter (`apps/api`) instantiates providers from environment config in `apps/api/src/registry/providers.ts`, then passes them to `createOngkirHub()` from `@ongkirhub/runtime`. Embedded consumers can do the same without the HTTP layer.
